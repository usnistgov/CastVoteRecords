...
Annotation is used to record annotations made by one or more adjudicators. The annotation can be specific to an entire CVR, or to a particular contest, contest selection, or indication. Multiple annotations per item can be included. [CVRSnapshot](#_17_0_2_4_78e0236_1389366224561_797289_2360) includes Annotation.
...
`Status`|0..*|`ContestStatus`|The status of the contest, e.g., overvoted, undervoted, from the [ContestStatus](#_18_0_5_43401a7_1475850124791_123384_4291) enumeration. If no values apply, use 'other' and include a user-defined status in OtherStatus.
...
`OtherStatus`|0..1|`String`|Used when Status is 'other' to include a user-defined status.
`Position`|0..1|`Integer`|Used to include the ordinal position of the ballot selection as it appeared on the ballot.
...
WriteIn is a subclass of CVRContestSelection and is used when the contest selection is a write-in. It inherits attributes from CVRContestSelection and has additional attributes for the image or text of the write-in.
